...
DeepScience 通过 MCP Connector 连接外部工具和服务。可以在 **Settings → Connectors** 添加本地命令或远程 HTTP Connector；配置保存在 `~/.deepscience/mcp.json`，由所有 Workspace 共用。

Connector 默认按需连接。Agent 只会看到一个轻量的 `mcp` 入口，在任务确实需要时再搜索、查看并调用远程工具，不会把所有 Connector 工具常驻到模型上下文。也兼容 `~/.config/mcp/mcp.json`、项目 `.mcp.json` 和 `.pi/mcp.json`。底层 MCP 运行时由 [`pi-mcp-adapter`](https://github.com/nicobailon/pi-mcp-adapter) 提供。
