---
name: biomarker-vax-vaccine-response
description: "Use the bundled VAX: vaccine response(msigdb C7) biomarker resource for Functional signatures, signature. Discover and read 4 local reference/data files on demand without loading the full collection into context."
category: biological-knowledge/functional-signatures/signature
license: Unknown
metadata:
  access-mode: local
  database: "VAX: vaccine response"
  collection: biomarker
  local-files: 4
  generated-from: resource_tree.json
---

# VAX: vaccine response(msigdb C7) Local Biomarker Resource

## Overview

Use DeepScience's bundled VAX: vaccine response(msigdb C7) collection for tasks involving Functional signatures, signature. This skill describes how to discover and read the local snapshot; it does not preload database contents.

## When to Use This Skill

Load this skill when a task needs VAX: vaccine response(msigdb C7) data or falls under **Functional signatures / signature**. Prefer the bundled snapshot for reproducible offline analysis. Use the official database only when the task requires newer records or functionality absent from the local files.

## Available Content

- Curated VAX: vaccine response(msigdb C7) material in the Functional signatures / signature collection

## Local Data

- **Data directory:** `packages/science/resources/skills/biological-knowledge/functional-signatures/signature/biomarker-vax-vaccine-response/assets/VAX_vaccine_response`
- **Reference directory:** `packages/science/resources/skills/biological-knowledge/functional-signatures/signature/biomarker-vax-vaccine-response/references`
- **Indexed local files:** 4
- **Combined size:** 0.85 MiB
- **Formats:** .gmt, .json, .md
