...
#### Official pathway gene-set export

The bundled `reactome_query.py gene-set` command reads the official release snapshot from
`assets/Reactome/ReactomePathways.gmt.zip`. Its GMT schema is pathway name, Reactome stable ID, then
gene symbols. Read [`gene-set-schema.md`](./gene-set-schema.md) for output artifacts and acceptance
checks. This is the default branch for pathway gene-symbol membership and does not use the Content
Service participant endpoints.

#### Connector participant expansion

The bundled `reactome_query.py participants` command uses:

```
GET /data/participants/{id}
```

It returns heterogeneous PhysicalEntity records with nested `refEntities`. This differs from a
flat gene-list response and from the event endpoint documented below. For the exact response shape,
reference classes, member-count definitions, and one-fetch parsing workflow, read
[`participants-schema.md`](./participants-schema.md).
