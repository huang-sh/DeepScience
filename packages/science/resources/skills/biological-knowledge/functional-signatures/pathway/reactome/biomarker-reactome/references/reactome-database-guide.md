...
The bundled connector's `participants` action uses `/data/participants/{id}`, whose response
contains heterogeneous PhysicalEntities and nested ReferenceEntities. Before deriving a member set
or count, read [`participants-schema.md`](./participants-schema.md). Do not treat either endpoint as
a flat gene-list API.
...
**For detailed API endpoints and parameters**, refer to `<resource-dir>/references/api_reference.md` in this skill.
...
For comprehensive API endpoint documentation, see `<resource-dir>/references/api_reference.md` in this skill.

## Release metadata

Reactome releases and database counts change regularly. Retrieve the active version rather than
relying on a value in this guide:

```bash
python3 <resource-dir>/scripts/reactome_query.py version
```

Report the retrieved version and access date with any pathway, reaction, participant, protein,
small-molecule, drug, or literature count.
