...
**Available Properties**: MolecularFormula, MolecularWeight, CanonicalSMILES, IsomericSMILES, InChI, InChIKey, IUPACName, XLogP, TPSA, HBondDonorCount, HBondAcceptorCount, RotatableBondCount, Complexity, Charge, and many more (see `<resource-dir>/references/api_reference.md` for complete list).
...
**For more complex bioactivity queries**, use the `<resource-dir>/scripts/bioactivity_query.py` helper script which provides:
...
### <resource-dir>/scripts/compound_search.py
...
### <resource-dir>/scripts/bioactivity_query.py
...
For detailed API documentation, including complete property lists, URL patterns, advanced query options, and more examples, consult `<resource-dir>/references/api_reference.md`. This comprehensive reference includes:
